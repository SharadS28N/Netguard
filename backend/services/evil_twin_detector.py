...
class EvilTwinDetector:
    ...
    def detect(self, networks: List[Dict]) -> Dict:
        ...
        combined_results = self._combine_results(networks, rule_results, ml_results)

        return combined_results

    def _combine_results(
        self, networks: List[Dict], rule_results: Dict, ml_results: Dict
    ) -> Dict:
        ...
        rule_suspicious = {
            n["bssid"]: n for n in rule_results.get("suspicious_networks", [])
        }
        ...
        overall_conf = 0.0
        if all_detections:
            overall_conf = sum(d["detection_score"] for d in all_detections) / len(
                all_detections
            )
        ...
        return {
            ...
            "detection_methods": {
                ...
                "ml_based": {
                    ...
                    "average_confidence": (
                        np.mean(ml_confidences) if ml_confidences else 0.0
                    ),
                },
            },
    ...
    def _generate_recommendations(
        self, evil_twins: List[Dict], suspicious: List[Dict]
    ) -> List[str]:
        ...
        if evil_twins:
            ...
            recommendations.append(
                "Recommended action: Use VPN, verify network with administrator"
            )
        ...
        if suspicious:
            ...
            for net in suspicious:
                recommendations.append(f"  • {net['ssid']} ({net['bssid']})")
            recommendations.append(
                "Recommended action: Verify network authenticity before connecting"
            )
