...
class FeatureExtractor:
    ...
    def _calculate_signal_variance(
        self, network: Dict, all_networks: List[Dict]
    ) -> float:
        """
    ...
    def _calculate_channel_congestion(
        self, network: Dict, all_networks: List[Dict]
    ) -> float:
        """
        ...
        """
        channel = network.get("channel", 0)
        networks_on_channel = sum(
            1 for n in all_networks if n.get("channel") == channel
        )
