...
class NetworkScanner:
    ...
    def get_available_interfaces(self) -> List[str]:
        ...
        try:
            interfaces = (
                scapy.get_windows_if_list()
                if sys.platform == "win32"
                else scapy.conf.ifaces.data.keys()
            )
    ...
    def enable_monitor_mode(self, interface: str) -> bool:
        ...
        try:
            if sys.platform == "linux" or sys.platform == "linux2":
                ...
                subprocess.run(
                    ["sudo", "airport", f"{interface}", "sniff"], check=False
                )
                return True
        except Exception as e:
    ...
    def parse_beacon(self, packet) -> Dict:
        ...
        try:
            if packet.haslayer(Dot11Beacon):
                ...
                # Get signal strength
                signal_strength = (
                    packet.dBm_AntSignal if hasattr(packet, "dBm_AntSignal") else -100
                )
        ...
        if packet.haslayer(Dot11):
            ...
            if packet.haslayer(Dot11Beacon):
                ...
                if network_info:
                    ...
                    self.networks[bssid]["signal_strength"] = network_info[
                        "signal_strength"
                    ]
    ...
    def start_scan(
        self, interface: str = None, duration: int = 30, channels: List[int] = None
    ):
