...
def test_layer_scores():
    ...
    test_signals = [
        ...
        {"layer": "ml", "is_outlier": True, "anomaly_score": -0.23},
    ]
    ...
    assert (
        0.49 < scores["signature"] < 0.51
    ), f"Signature score incorrect: {scores['signature']}"
    assert (
        0.66 < scores["behavior"] < 0.68
    ), f"Behavior score incorrect: {scores['behavior']}"
...
def test_confidence_computation():
    ...
    layer_scores = {"signature": 0.5, "behavior": 0.667, "ml": 1.0}
