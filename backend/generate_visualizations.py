...
def generate_synthetic_data(n_samples=2000):
    ...
    legitimate_features = np.column_stack(
        [
            ...
            np.random.choice(
                [0.8, 0.9, 1.0], legitimate_samples
    ...
    evil_twin_features = np.column_stack(
        [
            ...
            np.random.uniform(
                0.3, 0.8, evil_twin_samples
            ),  # channel_variance (unstable)
            np.random.choice(
                [0.0, 0.3, 0.5], evil_twin_samples
            ),  # encryption (Open/WEP)
...
def plot_model_accuracy_comparison(models, X_test, y_test):
    ...
    for i, (acc, prec, rec, f1) in enumerate(
        zip(accuracies, precisions, recalls, f1_scores)
    ):
        ax.text(
            i - 1.5 * width,
            acc + 0.02,
...
def plot_feature_importance(models, X_test):
    ...
    for idx, (name, model) in enumerate(tree_models.items()):
        ...
        axes[idx].set_title(
            f"{name.upper()} Feature Importance", fontsize=12, fontweight="bold"
        )
...
def plot_confidence_distribution(models, X_test):
    ...
    for idx, (name, model) in enumerate(models.items()):
        ...
        axes[idx].hist(
            confidence, bins=50, alpha=0.7, color="steelblue", edgecolor="black"
        )
        ...
        axes[idx].set_title(
            f"{name.upper()} Confidence Distribution", fontsize=12, fontweight="bold"
        )
...
def plot_detection_pipeline_metrics():
    ...
    # Processing time
    bars1 = ax1.bar(
        phases, avg_time, alpha=0.8, color=["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4"]
    )
...
def generate_summary_report(models, X_test, y_test):
    ...
    report_data = {"timestamp": datetime.now().isoformat(), "models": {}}
